...
def load_tabular_data(csv_path, batch_size=32, scale=True):
    ...
    if scale:
        ...
        # Return raw (unscaled) values — needed for LEM target derivation
        scaler.fit(X_train)          # still fit so scaler is returned fitted
    ...
    def to_tensor(dataX, datay):
        return TensorDataset(
            torch.tensor(dataX, dtype=torch.float32),
